...
%DIF DEL selac.old.tex   Wed Apr 18 13:03:23 2018
%DIF ADD selac.tex       Wed Apr 18 13:01:38 2018
...
Contrary to popular belief,  $\omega$ does not describe whether a site is evolving under a constant regime of stabilizing or diversifying selection, but instead how the selective environment changes over time under and even then under very limited conditions.
Given this behavior, $\omega$ based models only reasonably approximate a subset of scenarios such as over-/under-dominance or positive/negative frequency dependent selection \citep{HughesAndNei1988,Nowak2006}, where selection is perfectly symmetrical.
...
We do note that because \selac infers the optimal amino acid for each site, it is substantially more parameter rich than more commonly used models such as GTR$+\Gamma$, GY94, and FMutSel.
...
The third stage optimizes \DIFdelbegin \DIFdel{the model }\DIFdelend \DIFaddbegin \selac\DIFadd{'s }\DIFaddend parameters shared across the genome $\alphac$ and $\alphap$, and the sensitivity distribution parameter $\alphag$.
...
Ongoing work will address the need for speed, with the eventual goal of implementing \DIFdelbegin \DIFdel{the model }\DIFdelend \DIFaddbegin \selac \DIFaddend in popular phylogenetic inference toolkits, such as RevBayes \citep{revbayes}, PAML \citep{Yang2007} and RAxML \citep{Stamatakis2006}.
...
In fact, $\omega$ showed similar, though slightly reduced correlations, with the same empirical estimates of gene expression described above (Figure \ref{fig:OmegavsPsi})
 \marginpar{JEREMY: IS THERE AN ERROR IN FIGURE \ref{fig:OmegavsPsi}? THERE ARE NO $\omega$ VALUES $> 1$ AND (b-c) APPEAR TO HAVE A VALUE B/W LOG(-1.5) AND LOG(-1), BUT NOT a). }
...
\section{Discussion}
\DIFdelbegin \DIFdel{One goal of }\DIFdelend \DIFaddbegin \DIFadd{A central goal in }\DIFaddend evolutionary biology is to quantify the nature, strength, and, ultimately, shifts in the forces of natural selection relative to genetic drift and mutation.
As data set size and complexity increase, \DIFaddbegin \DIFadd{so does the amount of potential information on these forces and their dynamics.
As a result, }\DIFaddend there is a need for more complex and realistic models \citep{GoldmanEtAl1996,ThorneEtAl1996,GoldmanEtAl1998,HalpernAndBruno1998,LartillotAndPhilippe2004} to accomplish this.
\DIFaddbegin \DIFadd{Although extremely popular due to their elegance and computational efficiency, the utility of $\omega$ based models in helping us reach this goal is substantially more limited than commonly recognized.
Because they use only a single substitution matrix, they are only applicable for situations in which the substitution process and shifts in the selective environment are intrinsicly, such as with positive or negative frequency dependent selection; these models do not describe stabilizing or diversifying selection as commonly envisioned \mbox{%DIFAUXCMD
\citep[Oxford Encyclopedia Entry goes here][]{}}%DIFAUXCMD
.
}

%DIF > from Oxford Encyclopedia of Evolution ed. M Pagel
%DIF > ``Selection can be stabilizing (maintaining the local phenotype), directional (selecting for a different mean phenotype), or diversifying (favoring two or more different phenotypes). ``

\DIFaddend Starting with \citet{HalpernAndBruno1998}, a number of researchers have developed methods for linking site-specific selection on protein sequence and phylogenetics\citep[e.g.~][]{KoshiEtAl1999,DimmicEtAl2000,KoshiAndGoldstein2001,RobinsonEtAl2003,LartillotAndPhilippe2004,ThorneEtAl2012,RodrigueAndLartillot2014} 
\DIFdelbegin \DIFdel{. 
Our work here }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{HalpernAndBruno1998} }%DIFAUXCMD
calculated  a vector of 19 expected amino acid frequencies for each amino acid site, making it the most general and most parameter rich of these methods.
...
By grouping together amino sites with similar evolutionary behaviors, Lartillot and colleagues retained the descriptive nature of \mbox{%DIFAUXCMD
\citet{HalpernAndBruno1998} }%DIFAUXCMD
work while greatly reduced the number of model parameters needed \mbox{%DIFAUXCMD
\citep{LartillotAndPhilippe2004,RodrigueAndLartillot2014}}%DIFAUXCMD
.
}\selac \DIFaddend follows in this tradition \DIFaddbegin \DIFadd{of using multiple substitution matrices}\DIFaddend , but includes some key advances. 
\DIFaddbegin 

\DIFaddend First, by nesting a model of a sequence's cost-benefit function \Cost/\Func within a broader model\DIFaddbegin \DIFadd{, }\selac \DIFaddend allows us to formulate and test \DIFdelbegin \DIFdel{more specific biological hypotheses. 
For example, using a hierarchicalapproach we were able }\DIFdelend \DIFaddbegin \DIFadd{a hierarchical, mechanistic models of stabilizing selection.
More precisely, our nested approach allows us }\DIFaddend to relax the assumption that \PC deviations from the optimal sequence \aoptvec are equally disruptive at all sites within a protein. 
We found strong support for \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \selac\DIFadd{'s }\DIFaddend hypothesis that the strength of stabilizing selection against \PC deviations from \aoptvec varies between sites (\DeltaAICc XXXXXX). 
...
While the \DIFdelbegin \DIFdel{model we use to link amino acid sequence and protein function is simplistic}\DIFdelend \DIFaddbegin \Cost\DIFadd{/}\Func \DIFadd{function on which }\selac \DIFadd{currently rests is very simple, nevertheless}\DIFaddend , it leads to a \DIFdelbegin \DIFdel{substantial }\DIFdelend \DIFaddbegin \DIFadd{dramatic }\DIFaddend increase in our ability to explain the sequence data \DIFdelbegin \DIFdel{used to parameterize the model.
Our model is built on }\DIFdelend \DIFaddbegin \DIFadd{we analyzed.
Importantly, because }\selac \DIFadd{uses }\DIFaddend a formal description of a sequence's \Cost/\Func,  \DIFdelbegin \DIFdel{which make modifying our assumptions }\DIFdelend \DIFaddbegin \DIFadd{replacing our assumptions with more sophisticated ones in the future is }\DIFaddend relatively straightforward. 
\DIFaddbegin \DIFadd{Conceptually, our work lies in between that of Lartillot's and Thorne's, where the latter is utilizing even more detailed models of protein structure as a means of linking amino acid substitutions and stabilizing selection.
}\DIFaddend Third, our use of nested models also allows us to make biologically meaningful and testable predictions. By linking a gene's expression level to the strength of purifying selection it is experience, we are able to provide coarse estimates of gene expression. When we apply this insight to the standard models, we found that $\omega$ is best explained as a proxy for gene expression, rather than the nature of selection on a sequence.
\DIFaddbegin 

\DIFaddend One simplifying assumption we make is that the organism can and does compensate for any reduction in protein function by simply increasing the protein's production rate.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend While this production compensation assumption will clearly not hold in many situations, it does allow us to connect protein function and energetic costs in a simple and biologically plausible manner. 
...
One organismal response, the one envisioned here, is that the protein is thus produced far more to compensate. 
...
In biology, it is likely a mixture of such effects exists. 
However, the production compensation mechanism is likely to have the same costs across proteins, making it a useful first approximation to model, while the same expression but reduced functionality will have gene specific effects more difficult to model generally (e.g., how does the cost of having glucose transport slow by half compare to the cost of underproducing an anthocyanin for flower color or fewer taste receptor proteins?). 
Moreover, there is evidence that cells do compensate for lower protein function by increasing gene expression \citep[MANY GOOD CITATIONS]. 
...
Most worrisome are biological flaws in \DIFdelbegin \DIFdel{the model}\DIFdelend \DIFaddbegin \selac\DIFaddend .
For example, at its heart, \DIFdelbegin \DIFdel{the model }\DIFdelend \DIFaddbegin \selac \DIFaddend assumes that suboptimal proteins can be compensated for, at a cost, simply by producing more of them. 
...
\DIFdelbegin \DIFdel{For example, it is possible to extend our model to include a hidden Markov modelling (HMM) approach, which would allow for shifts in the optimal/pessimal amino sequence along any given branch in the phylogeny. 
Extending our model in this way, should eventually allow researchers to describe not only the dynamic shifts in natural selection throughout the phylogeny, but evaluate how well a given dataset supports such a model.
}\DIFdelend 
...
\DIFdelbegin \DIFdel{Overall}\DIFdelend \DIFaddbegin \DIFadd{In spite of these difficulties}\DIFaddend , \selac represents an important step in uniting phylogenetic and population genetic models.\DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{While \mbox{%DIFAUXCMD
\citet{KoshiEtAl1999,DimmicEtAl2000,KoshiAndGoldstein2001,RobinsonEtAl2003,LartillotAndPhilippe2004,ThorneEtAl2012,RodrigueAndLartillot2014} }%DIFAUXCMD
are all models of constant, stabilizing selection, }\selac \DIFadd{can be generalized further to include diversifying selection.
Specifically, by letting }\selac\DIFadd{'s Grantham weighting term $G$, which we now assume is $\ge 0$,  to take on negative values, }\selac \DIFadd{now behaves as if there is a pessimal, rather than optimal, amino acid for the given site.
In this diversifying selection scenario, amino acids with }\PC \DIFadd{qualities more dissimilar to the pessimal amino acid are increasingly favored, potentially resulting in multiple fitness peaks.
...
%DIF > Fortunately, it is also a question that can be addressed using \selac in the future.
...
Due to its mechanistic nature, }\selac \DIFadd{can also be extended to include changes in the optimal/pessimal sequence over a  phylogeny using a hidden markov modelling approach.
Extending }\selac \DIFadd{in these ways, will allow researchers to model only shifts in natural selection and, in turn, quantify their frequency and magnitude.
}

%DIF > It is worth noting that allowing $G < 0$ is not equivalent to choosing a different amino acid as being optimal. 
%DIF > To see this, imagine a focal amino acid with alternative two amino acids equally distant from it in \PC but in opposite directions.
%DIF > If the focal amino acid is the pessimal amino acid for the site (i.e.~$G<0$), then the two alternative amino acids represent two alternative fitness peaks in different regions of the \PC.
%DIF > Treating one of the alternative amino acids as optimal (i.e.~$G > 0$) results in the other alternative amino acid having even lower fitness than the focal one; a distinctly different scenario than the pessimal case.

 

\DIFadd{In summary, }\selac \DIFaddend allows biologically relevant population genetic parameters to be estimated from phylogenetic information, while also dramatically improving fit and accuracy of phylogenetic models. 
\DIFaddbegin \DIFadd{By explicitly modeling the optimal/pessimal sequence of a gene, }\selac \DIFadd{can be extended to include shifts in the optimal/pessimal sequence over evolutionary time.
...
}\DIFaddend Moreover, it demonstrates that there remains substantially more information in the coding sequences used for phylogenetic analysis than other methods can access. 
...
This includes not only the parameters in \DIFdelbegin \DIFdel{the model}\DIFdelend \DIFaddbegin \selac\DIFaddend , but also the optimal amino acids for a given sequence as well as the estimates of the branch lengths.
...
However, the inclusion of $\alphag$ into \DIFdelbegin \DIFdel{the model}\DIFdelend \DIFaddbegin \selac\DIFaddend , appears to generally increase values of $\phi$ and generally improves the ability to recover the optimal amino acids even for the gene with the lowest baseline $\phi$.
...
A reviewer pointed out that it may also be difficulty for \DIFdelbegin \DIFdel{the model }\DIFdelend \DIFaddbegin \selac \DIFaddend to account for changing amino-acid, which we agree may also play a role.
