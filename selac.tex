...
These \PC based codon models as originally introduced have rarely been used for empirical data \citep[for exceptions see][]{CITATIONS}.\marginpar{Cite papers by authors mentioned by reviewer 2 (McClellan, Mackiewicz, Yang, Zhang, Goldstein, Higgs and presumably TamuriEtAl2009) here?}
...
\selac models also appeared to outperform, based on likelihood, and reported AIC and AICc from each program, the codon models in IQtree. %citation
Supplemental table \_\_\_\_ compares these. %add table
...
Further, by comparing likelihood values of models common to both \selac and IQtree, we verified that IQtree takes a similar approach.\marginpar{mike: this contradicts what we say in our response to reviewers letter.}
A result, the log-likelihood values between \selac and IQtree are comparable, although some minor differences in model implementation can lead to small differences in log-likelihood values
%, though Because of minor differences in model implementations one should expect some differences between likelihood values between models in \selac and IQtree, values between similar model..
