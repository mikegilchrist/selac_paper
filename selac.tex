...
However, phylogeneticists have long been guilty of using a statistical sleight of hand by ignoring the fact that majority rule states are parameters and these parameters are estimated from the data.
Of course, this does not matter when comparing nucleotide models with AIC because the difference in $\K$ when counting or not counting number of sites drops out in the comparison.
It does, however, matter for AICc, where the number of parameters, $\K$, and the sample size, $\n$, combine in the penalty term.
...
In comparative methods (e.g., Butler and King 2004, O'Meara et al. 2006, Beaulieu et al. 2013) the number data points, $\n$, is taken as the number of taxa.
More taxa allow the fitting of more complex models, given more data.
However, in DNA evolution, which is effectively the same as discrete character model used in comparative methods, the $\n$ is taken as the number of sites.
Obviously, both cannot be correct.
The original derivation of AICc by Hurvich and Tsai (1989) assumed a regression model, where the true model was in the set of examined models, as well as approximations in the derivation itself.
...
Second, when considering how likelihood is calculated, for a given site it is the sum of the probabilities of each observed state at each tip, and this is then multiplied across sites.
...
An exception is _____SURFACE___, which uses multiple characters and taxa.
...
Recently, Jhwueng et al. (2014) performed an analysis that investigated what variant of AIC and AICc worked best as an estimator, but the results were inconclusive.
Here, we have adopted and extended the simulation approach of Jhwueng et al. (2014) in order to examine a large set of different penalty functions and how well they approximate the remaining portion of Kullback-Liebler divergence between two models after accounting for the deviance (i.e., $-2\Lik$).
...
For our analysis to suggest what the best measure of dataset size is for AICc, it suggested using a scaled value of the product of number of sites and number of characters was the best at estimating KL distance. See Appendix 1 for more details.
...
For likelihood models, use of AICc has become standard, but the conflict in how number of data points is calculated remains in the field.
Common sense suggests that dataset size is increased by adding taxa and/or sites: a dataset of 1000 taxa and 100 sites must have more information on substitution models than a dataset of 4 taxa and 100 sites.
...
For example, one could imagine a phylogeny where one taxon is sister to a polytomy of 99 taxa, where each of the 99 has a zero length terminal branch.
...
This is just one complexity; we expect there are many more, so a deeper investigation of this is warranted.
...
\section{Table}
  ...
  \begin{table}[H]
    \begin{tabular}{lrrrrl}
                				&		   &Parameters &              &              &            &    Model\\
      Model                 	& logLik   & Estimated &           AIC&          AICc&   \DeltaAIC&  Weight\\\hline
