...
\item Connection to past work
  ...
  \item There are many kinds of selection: towards some optimal protein structure, to reduce cost of synthesis, towards robustness to errors, etc.
  \item There are models that simplify this in various ways...
  \end{itemize}
\item Confusion over interpreting $\omega$
\item Confusion about number of parameters addressed by KL
  \begin{itemize}
...
\item Key results
  \begin{itemize}
\item Connection between genotype, energy flux and fitness are simplistic
  \begin{itemize}
  ...
  \end{itemize}
\item Multiple ways to derive our equations on first principle.
 ...
 \item Assume gene regulation monitors reaction rates or close proxy and compensates expression accordingly.
   Simplistic, but leads to simple relationship
 \end{itemize}
\item  Can be refined further based on researchers' biological knowledge and interests.
  E.g.
\begin{itemize}
\item more complex distance function (non-linear, add indicator functions, further categorize proteins or domains within proteins for different weighting functions, sensitivity distributions, etc)
\end{itemize}
\end{itemize}
\end{itemize}
...
\item Quality of predictions likely due
  ...
  \item Similarly, we could add a term to our distance function that acts as an indicator function and then estiamte its weight.
    This would allow us to more directly evaluate different hypotheses about whether it's the \PC properties that matter or simply the fact that the aa are different.
    \end{itemize}
...
There are many models of DNA substitution in molecular phylogenetics. The most popular are simple nucleotide models which are naturally agnostic with regards to the different amino acid substitutions and their impact on gene function (e.g.~F81, F84, HYK85, TN93, and GTR, see \citet{Yang2014} for an overview). However, a set of more complex models, starting with \citep{GoldmanAndYang1994}, take into account the underlying processes of mutation and selection. \citep{GoldmanAndYang1994} start by explicitly using distance between amino acids as a factor that affects relative substitution rates. More recent papers simplify this to just estimating a parameter, $\omega$, for the nonsynonomous/synonomyous substitution rate.
...
A ribosome is an expensive piece of cellular machinery that has a finite lifespan, so requiring more use to produce more proteins is also expensive.
Additional costs are costs of transcription, chaperones to help with protein folding, movement of the protein to its final destination, as well as factors like speed of translation (which is affected by things like tRNA abundance and codon wobble).
In addition to these more direct costs, transcription and translation errors that lead to either the reduction of protein production rates or the functionality of the protein product can result in indirect costs.
Translation errors, in particular, are thought to contribute lead to different translational efficencies between synonymous codons and, in turn, shape sequence evolution \citet{Kurland1991,Gilchrist2007,PlaceHolderRef}.

Another key difference from existing models is the symmetry of rates.
A common simplifying assumption under traditional models, even with selection, is that the rate at which a codon i to j mutation is fixed is the same as the rate at which a codon j to i mutation is fixed (though the frequency with which these mutations are proposed by evolution may not necessarily be equal).
This overall rate is, in some models (\_\_\_\_\_), but not others (\_\_\_), related to a measure of distance of amino acid properties as a measure of difference in fitness.
...
[mikeg: NIELSENANDYANG2003 INCLUDES NE AND S BUT DOESN"T SEEM TO TALK ABOUT WHAT OMEGA MEANS.
NEED TO CHECK]
[BCO: Goldman Yang 1994 doesn't explicitly include omega]


Perhaps because of its elegance and simplicity, a wide range of modifications have been incorporated into the GY94 model. %, such as allowing for variation in $\omega$ along a branch (as cited in Anisimova 2012)
