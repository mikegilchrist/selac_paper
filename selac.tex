...
With respect to estimates of $\phi$ within SelAC, they were strongly correlated with two separate empirical measurements of gene expression (See Figure \ref{fig:PhivsROC}; Figure S1-S2\footnote{mikeg: Please add these figures.}).
...
\footnote{mikeg: ROC is NOT an 'empirical' based estimate of gene expression.  
  The mRNA and microarray data should be used instead and this figure should be put in the supporting materials.} 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{FIGURE_2_MutSelOmega_vs_Us_ROC_Scer_only.pdf}
  \caption{Comparisons between $\omega$, which is the nonsynonymous/synonymous mutation ratio in FMutSel0, $\phi$ obtained from \selacplusgamma (a), and two empirical estimates of gene expression for individual taxa across the 100 selected genes from the yeast data set of Salichos and Rokas (2013).  
...
\footnote{mikeg: For (a) shoudln't it be $\psi$, which is not genome specific, rather than $\phi$, which is.  
      Is the RNA seq and ROC data for brewer's yeast? 
  } 
...
\footnote{mikeg: Can you simply put the names of the model next to the traces?  As it stands now \selac is ambiguous.
      In addition, is this \selac or \selacplusgamma?
  } 
...
First, the ability to accurately recover the true optimal amino acid sequence will largely depend on the strength of $\phi$ of the gene.
