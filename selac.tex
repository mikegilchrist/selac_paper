...
The simplest but most popular models assume neutrality between the different amino acid substitutions and may or may not include mutation bias (e.g.~F81, F84, HYK85, TN93, and GTR for the former and JC69 and K80 for the latter, see \citet{Yang2014} for an overview).
...
We model the substitution process as a classic Wright-Fisher process which includes the forces of mutation, selection, and drift \citep{Fisher1930,Kimura1962,Wright1969,Iwasa1988,BergAndLassig2003,SellaAndHirsh2005,McCandlishAndSoltzfus2014}.
...
For our purposes, we rely on the general unrestricted model (UNREST) \citep{Yang1994} because it is time irreversible and makes no constraint on the instantaneous rate of change between any pair of nucleotides.\footnote{mikeg: I am unsure wwhy is this model is being described as time irreversible.
My understanding is that its stationary distribution it satisfies the condition $\pi_i P_{i\rightarrow j} = \pi_j P_{j\rightarrow i}$ and thus classified as a time reversible markov process.
Are you confusing 'general time reverisble', a phylo specific term, with 'time reverisble'?
}
...
These equilibrium nucleotide frequencies are determined through analytically solving $\pimatrix \times \Qmatrix = 0$. \footnote{mikeg: are my updates correct?}
We use this\footnote{mikeg: It is unclear if `this' is referring to the \pimatrix or the \Qmatrix} to populate a $64 \times 64$ codon mutation matrix $\mumatrix$, whose entries $\muij$ describe the mutation rate from codon $i$ to $j$ under a "weak mutation" assumption.
...
As a result, we set $\alphav = 3.990333 \times 10^{-4}$\footnote{mikeg: does Grantham really report 7 digits of precision?}, the value originally estimated by Grantham, and recognize that our our estimates of $\alphac$ and $\alphap$ and $\psi$ are scaled relative to this choice for $\alphav$.
More specifically,
\begin{equation*}
...
\end{equation}
In this case, the data, $\Dmatrixp$, are the observed codon states at position $p$ for the tips of the phylogenetic tree  $\Tmatrix$.
For our purposes we take \Tmatrix as given but it could be estimated as well.
The pruning algorithm of Felsenstein (1981) is used to calculate $\Lik(\Qmatrixa)$.
...
This entire procedure is repeated six times or until the likelihood between successive rounds is less than a specified tolerance (the default is 0). \footnote{
mikeg: As I recall, you currently check to make sure the model likelihood is not changing much between the 5th and 6th round of optimization.
You should indicate this fact here.
Otherwise, this suggests we may not have completed the optimization process adequately.
}
...
In other words, we examined whether data simulated under the MLE produces properties similar to the input sequence data (e.g., Beaulieu et al., 2013).
...
For comparison, we also conducted the same test, by simulating the sequence under the standard GTR + $\Gamma$ nucleotide model, which is often used on these data, but does not account for the fact that the sequence codes for a specific protein.
