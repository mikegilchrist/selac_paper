...
What was considered our "known" parameters under a given generating model were based on empirical analyses of a 106 gene data set and phylogeny of yeast from Rokas et al. (2003). 
The tree used in these analyses is outdated with respect to the current hypothesis of relationships within Saccaromyces (see section on Analyzing Yeast Genome), but we rely on it simply as a means for examining the behavior of our model.
Here, we focused these analyses on the seven Saccaromyces taxa, and thus, we removed Candida albican from the Rokas et al. (2003) tree. 
...
Each of these data sets were analyzed in R, and the MLE estimate for a given replicate were taken as the fit with the highest log-likelihood after running five independent analyses with different sets of naive starting points with respect to the composite $\C\q\psi\Ne$ parameter, $\alphac$, and $alphap$.  
