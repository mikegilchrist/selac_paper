...
%\documentclass[webpdf,mynatbib,surname,CE,MSC]{SYS-PV}
\documentclass{SYS-PV}
% when compiling use option: -include-directory=Sys.Bio.LaTeX
...
\author[]{J\textsc{EREMY} M.~\surname{B\textsc{EAULIEU}}$^{1,2,3}$,
J\textsc{UAN JUAN} \surname{C\textsc{ROSSKEY}}$^{2,4}$\footnote{mikeg: Brian, did you ever get a hold of JJ?} 
C\textsc{EDRIC} \surname{L\textsc{ANDER}}$^{2,3}$, 
R\textsc{USSELL} \surname{Z\textsc{ARETZKI}}$^{5}$, 
B\textsc{RIAN} C.~\surname{O'\textsc{MEARA}}$^{2,3}$, 
\textsc{AND}
M\textsc{ICHAEL} A.~\surname{G\textsc{ILCHRIST}}$^{2,3\ast}$}

\address{$^1$Department of Biological Sciences, University of Arkansas, Fayetteville, AR 72701
...
The gene specific parameter $\psi_g$ is multiplied by additional model terms to make a composite term $\psiprime_g$ which scales the strength and efficacy of selection for the optimal amino acid sequence relative to drift.\footnote{mikeg: Note that we seem to have eschewed the prime for $\psi$ and $\phi$ in the figures.}
...
For \selac, we compared our estimates of $\phiprime = \psiprime/\Func$, which represents the average protein synthesis rate of a gene, to estimates of gene expression from empirical data.\footnote{mikeg: added $\prime$ to $\phi$ since that seems to be what we are using with \selac. 
Seems like we could drop the $\hat{}$ in the figures and add the $\prime$ \emph{if} Jeremy wants to.}
...
For further comparison, we also predicted protein synthesis rate ($\phi$) by analyzing gene and genome-wide patterns of synonymous codon usage using ROC SEMPPR (\citet{GilchristEtAl2015}) for each individual genome.
...
Nevertheless, ROC SEMPPR predictions of gene expression $\phi$ correlates strongly ($\r = 0.6-0.7$) with experimental measurements of gene expression \citet{GilchristEtAl2015}.
...
Our estimate of $\alphag$ = 1.40, produced a distribution of sensitivity terms $G$ ranged from 0.344-7.16, but with nearly 90\% of the weight for a given site-likelihood being contributed by the 0.344 and 1.48 rate categories.\footnote{mikeg:Jeremy, can you send me these \#'s?}
...
Perhaps more convincingly, we find that the target expression level $\psi$ and realized protein synthesis rate $\phi$ are reasonably well correlated with laboratory measurements of gene expression ($\r = 0.34-0.65$; Figure \ref{fig:phiComparisons}, Figure S1-S2)).
...
\begin{figure}[H]
  ...
  \caption{Comparisons between estimates of $\phi$ obtained from \selacplusgamma and direct measurements of expression for individual yeast taxa across the 100 selected genes from \citet{SalichosAndRokas2013}.
  	...
  	Gene expression was measured using either RNA-Seq (a-c) or Microarray chips (d), and the equations in the upper left hand corner of each panel represent the regression fit and correlation coefficient $r$.\textbf{mikeg: Figures have $r$ instead of $\rho$ as mentioned in text.}
  } 
