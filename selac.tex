...
For example, \selac prediction of gene specific protein synthesis rates correlates well with both empirical (Pearson $r = 0.34-0.48$) and other theoretical predictions (Pearson $r=0.59-0.64$) for multiple species.
\selac also provides quantitative estimates of probability a given amino acid is optimal for a given site.\marginpar{MIKE: Do we ever present such results?}
...
However, the model's actual behavior is inconsistent with how these terms are typically defined and understood \citep[e.g.~see][]{Pellmyr2002}. %\footnote{Although \citet{GoldmanAndYang1994} use a more complex, \PC based distance function instead of $\omega$, the criticisms below also apply to this work.}.
...
The selection against the resident amino acid $i$ persists until a substitution occurs at which point selection now \emph{favors} amino acid $i$, as well as the 18 other amino acids, to the same degree $i$ was previously disfavored.
...
\marginpar{MIKE: Is this definition of $\mu_{i,i}$ correct?}
...
More explicitly, $ S = \left(\sum_{i \in \text{codons}} -\mu_{i,i} \pi_{i,i}\right)$ where the final mutation rate matrix is the original mutation rate matrix multiplied by $1/S$. \marginpar{MIKE: I updated the definition of $S$. 
...
Compensatory changes that allow an organism to maintain functionality even with loss of one or both copies of a gene are widespread (reviewed in \citet{PLACEHOLDER}\marginpar{MIKE: Brian, please provide references or cut.}
\footnote{From Cruft: 
...
\end{equation}
where $n$ is the length of the protein, $d(a_p, \aoptp)$ is a weighted physicochemical distance between the amino acid encoded in a given at position $p$ and $\aoptp$ is the optimal amino acid for that position.
...
\marginpar{MIKEG: Jeremy, is this still true? 
I thought the final state was assuming they summed to 1.}
More specifically,
\begin{align*}
...
\marginpar{MIKE: I thought we updated this to be based on the change in \LLik function.}
...
\marginpar{MIKE: Can someone verify this last statement about FMutSel0?}
...
\marginpar{MIKE: This sentence doesn't make any sense to me.}
...
% \marginpar{MIKE: JEREMY IS THERE AN ERROR IN FIGURE \ref{fig:OmegavsPsi}? THERE ARE NO $\omega$ VALUES $> 1$ AND (b-c) APPEAR TO HAVE A VALUE B/W LOG(-1.5) AND LOG(-1), BUT NOT a). }
...
Perhaps more convincingly, we find that the target expression level $\psi$ and realized average protein synthesis rate $\phi$ are reasonably well correlated with laboratory measurements of gene expression (Pearson $r= 0.34-0.65$; Figures \ref{fig:PhivsEmpirical}, \ref{fig:PhivsROC}, and \ref{fig:ROCvsEmpirical}). 
