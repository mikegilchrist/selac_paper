...
\begin{align}
  \label{eq:fSeriesDef}
  f(d) &= 1 + \sum_{k=1}^\kmax \frac{1}{k!}\frac{d f^k}{d^k d} d^k + O(d^{\kmax+1})\\
  & = 1 + \sum_{k=1}^\kmax A_k d^k + O(d^{\kmax+1})\\
\end{align}
...
[REMAINING QUESTION: Where is our grantham $g$ sensitivity term in all of this?  
I remember we concluded we shouldn't expect it to vary systematically with $\phi$ so it seems like it is already part of the $f(d)$ function.
Additionally, how does treating $g \sim \text{Gamma}$ change things? 
Does it some how absorb our uncertainty in $\phi$ or $A_k$?
]
...
\end{align*}
where $b=1$ for a diploid population and $2$ for a haploid population \cite{Iwassa88,BergAndLassig03,SellaAndHirsh05}.
Finally, assuming a constant the mutation between alleles $i$ and $j$ is $\mu_{i,j}$, the transition rate from $i$ to $j$ is,
\begin{align*}
...
\noindent [NOTE: NEED TO DEFINE $\mu$]
