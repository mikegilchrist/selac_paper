...
Log scale?  Choosing relative scale for Ne s  
    ...
    \item Shortcomings $\rightarrow$ future work
      ...
      \item Fundamental model assumptions
        ...
        \end{itemize}
      \item Yeast having problems with estimating kluyveri branch.
        Are any of these gene in CLeft
      \item Estimating uncertainty is challenge
      \item Break current juggernaut of $\omega$
      \item Not currently integrated with other  
      \item Identifiability issues
      \item Issues with discreteness of amino acids
      \item Fixed amino acid, but easy to extend
      \item Likelihood based estimate ancestor state rather consensus assumption. 
      \end{itemize}
    \item Extensible to other researchers approaches
      \begin{enumerate}
      \item Structure information
      \item Selection on CUB
      \item Approach can even be expanded to other types of sequence data
      \end{enumerate}
    \item Extract additional data
      \begin{itemize}
      \item Gene expression.
      \item functionality: $\phi$ vs $\psi$
      \item Extension of the model to include transitions between the optimal aa for a site, still satisfies criteria of detailed balance at equilibrium so still a time reversible process.
      \end{itemize}
