...
However, the model's actual behavior is inconsistent with how these terms are typically used \citep{Pellmyr2002}. %\footnote{Although \citet{GoldmanAndYang1994} use a more complex, \PC based distance function instead of $\omega$, the criticisms below also apply to this work.}.
...
By definition, the optimal amino acid sequence for a given gene, \aoptvec, produces one unit of functionality, i.e.~$\Func(\aoptvec) = 1$.
...
As a result, it follows that $\phi = \psi/\Func(\avec)$ and the cost of a suboptimal amino acid increases the more it decreases the protein's functionality, \Func.
In other words, the average production rate of a protein \avec with relative functionality $\Func(\avec) < 1$ must be $1/\Func(\avec)$ times higher than the production rate needed if the optimal amino acid sequence \aoptvec was encoded since $\Func(\aoptvec) = 1$.
...
Nevertheless, ROC-SEMPPR predictions of gene expression $\phi$ correlates strongly ($\rho_\text{Pearson} = 0.53-0.74$) with a wide range of laboratory measurements of gene expression \citep{GilchristEtAl2015}.
...
The appropriateness of this approximation for phylogenetic data, where shared evolutionary history means data points between taxa lack independence.
