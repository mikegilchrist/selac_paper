...
Another set of models attempt to include a `selection' term $\omega$, but the link between $\omega$ and the key parameters found in standard population genetics models such as \Ne and the distribution of fitness across genotype space is far from clear.
...
Thus, the simplest and most consistent interpretation of $\omega$ is that it represents the rate at which the selection regime itself changes, and this change in selection perfectly coincides with the fixation of a new amino acid.
...
% models, such as the cost-benefit one we present here.

% that models which include codon specific effects, which presumably affect protein assembly costs, and amino acid specific effects, which presumably affect protein functionality, inherently leading to a model with epistatic interactions between sites.
...
The gene specific parameter $\psi$ is multiplied by additional model terms to make a composite term $\psiprime$ which scales the strength and efficacy of selection for the optimal amino acid sequence relative to drift (see \nameref{sec:implementation}).
...
TODELETE: That is, the rate of allele fixation is greater than \Nemu \marginpar{Shouldn't this be `$1/(2 \Ne)$ generations'? JMB: I agree -- did not make change until Brian confirms. BCO: I don't agree: all we mean by weak mutation is no more than two alleles per site. I'd cut everything at "and $\Nemu \ll 1$" and following}  and $\Nemu \ll 1$, such that evolution is mutation limited, codon substitutions only occur one nucleotide at a time and, as a result, the rate of change between any pair of codons that differ by more than one nucleotide is zero.
...
More explicitly, $ S = \left(\sum_{i \in \text{codons}} \mu_i \pi_i\right)$.
\marginpar{I assume the $-$? in the previous version was a typo, correct?. JMB: No, it should be negative. Brian -- you agree?. BCO: Line 1081 of selac.R, so should be positive. It's neg originally b/c we're taking the neg of the diag, which itself is neg of the rowsums}
Thus, The final mutation rate matrix is the original mutation rate matrix multiplied by $1/S$, and represents one expected proposed mutation per site.
\marginpar{Are my corrections here correct? JMB: No. Brian -- you agree.? BCO: I'd delete the "and represents one expected proposed mutation per site" b/c it's actually 1 unit of brlen = 1 mutation (nothing about time in the original sentence) but it's too kludgy to write this.}
...
Further, at the limit of $\alphag \rightarrow \infty$, the model becomes equivalent to assuming model uniform sensitivity site sensitivity where  $\gp = 1$ for all positions $p$.
...
For example, a cell with an allele \avec where $\Func(\avec) = 0.9$ will have to produce $10/9 = 1.11\%$ times the proteins that a competitor cell with the optimal allele \aoptvec would have to produce.
Similarly, a cell with an allele \avec where $\Func(\avec) = 1/2$ will have to produce $2/1 =  2$ times the proteins that a cell with \aoptvec would have to produce.
...
This results in the creation of 20 \Qmatrix matrices, one for each amino acid and each with $3721$\marginpar{where does this 26,880 come from? JMB: No idea. Good catch. I have 61*61=3721. Made this change. Brian -- you agree? BCO: But does it really have 61^2 UNIQUE rates?} unique rates, based on few parameters (one to 11 mutation rates, two free Grantham weights, the cost of protein assembly, $A_1$ and $A_2$, the gene specific target functionality synthesis rate $\psi$, and optimal amino acid at each position $p$, \aoptp), which can either be specified \emph{a priori} or  estimated from the data.
\selac can be generalized to allow transitions between optimal amino acids as well as between codons, which would result in a $(20 \times 64) \times (20 \times 64) =  1344 \times 1344$ matrix.\marginpar{Is this worth mentioning?  If so why here instead of in Discussion? JMB: Yeah, this seems a bit out of place. I would just cut completely. BCO: Agree, cut}
...
\marginpar{I thought we used 8 points. JMB: No. Takes way too long. Had to go with 4. BCO: Ok}
...
This entire four stage cycle is repeated six times.
...
\marginpar{Is this $<10$ accurate? JMB: Yes. Our yeast example is less than 10 species and it took forever. BCO: OK}
This is largely due to the size and quantity of matrices we create and manipulate to calculate the log-likelihood of an individual site.
...
The tree used in these analyses is outdated with respect to the current hypothesis of relationships within \emph{Saccharomyces}, but we rely on it simply as a training set that is separate from our empirical analyses (see \nameref{sec:analysis}).
...
The FMutSel0 model, which assumes that the amino acid frequencies are determined by functional requirements of the protein, but where XXXX, is the most similar to our model.
\marginpar{Can you fill in the XXX? JMB: I do not understand what is being communicated here. BCO: I don't get it, either}
...
Gene expression in this context corresponds to mRNA abundances which were measured using either Microarray chips (\emph{C. glabrata}, \emph{S. castellii}, and \emph{S. kudriavzevii}) or RNA-Seq (\emph{S. paradoxus}, \emph{S. mikatae}, and \emph{S. cerevisiae}).
...
Here, we have adopted and extended the simulation approach of \citet{Jhwuengetal2014} in order to examine a large set of different penalty functions and how well they approximate the remaining portion of Kullback-Liebler (KL) divergence between two models after accounting for the deviance (i.e., $-2\Lik$) (see Appendix 1 for more details).
...
%We leave the identification of such conditions and approximations for future studies.
...
JMB was supported, in part, as a Postdoctoral Fellow at the National Institute for Mathematical and Biological Synthesis, an Institute sponsored by the National Science Foundation through NSF Award DBI-1300426, with additional support from UTK.
