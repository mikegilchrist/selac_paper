...
This is in spite of the need for estimating the optimal amino acid at each position in each protein, which adds more than 47,000 more parameters to the model than YN08, the next most parameter rich codon model in our model set, and yet the SelAC model shows nearly 400,000 AIC unit improvement over YN08.
...
Other parameters in the model, such as the Grantham weights, provide an indication as to the physiochemical distance between amino acids.
...
Similarly, other physiochemical properties besides composition, volume, and charge likely contribute to site specific patterns of amino acid substitution.
...
Even if other physiochemical properties are considered, the idea of a consistent, genome wide Grantham weighting of these terms seems highly unlikely.
...
While \selac assumes that functionality declines with physiochemical distance from the optimum amino acid, as it stands this decline is less dramatic than one might intuitively expect.
...
Thus, while our assumption of about the link between functionality and physiochemical distance may be reasonable for sequences close to the optimum sequence \aoptvec, it clearly fails for protein sequences far from the optimum.
...
In addition, \selac naturally lends itself to model selection since we can compare the quality of \selac fits using different mixtures of physiochemical properties.
...
mikeg: I've removed the extra digits.}, the value originally estimated by Grantham, and recognize that our our estimates of $\alphac$ and $\alphap$ and $\psi$ are scaled relative to this choice for $\alphav$.
More specifically,
\begin{equation*}
