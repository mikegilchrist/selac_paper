...
%\marginpar{Shouldn't this be `$1/(2 \Ne)$ generations'? JMB: I agree -- did not make change until Brian confirms. BCO: I don't agree: all we mean by weak mutation is no more than two alleles per site. I'd cut everything at "and $\Nemu \ll 1$" and following.
%RESOLVED: Implemented BCO's recommendation.
%Note that this is the mutation part of the model, not the substitution part, which I've updated.} 
...
%\marginpar{I assume the $-$? in the previous version was a typo, correct?. JMB: No, it should be negative. Brian -- you agree?. BCO: Line 1081 of selac.R, so should be positive. It's neg originally b/c we're taking the neg of the diag, which itself is neg of the rowsums. RESOLVED: $-$ has been removed.}
...
MAG: I think it's important to be clear about units and think my solution isn't very kludgy (though may be incorrect).}
...
\end{align*}
where, given weak mutation assumption, $\muij = 0$ when two codons differ by more than one nucleotide and $\Ne \mu \ll 1$.
\marginpar{MAG: Added weak mutation math here.}
...
%\marginpar{I thought we used 8 points. JMB: No. Takes way too long. Had to go with 4. BCO: Ok. MAG: Dang.  I really think this is a likely source of error. RESOLVED: 4 is correct}
