...
Because they use only a single substitution matrix, they are only applicable for situations in which the substitution process and shifts in the selective environment are intrinsicly, such as with positive or negative frequency dependent selection; these models do not describe stabilizing or diversifying selection as commonly envisioned \citep[Oxford Encyclopedia Entry goes here][]{}.

%from Oxford Encyclopedia of Evolution ed. M Pagel
%``Selection can be stabilizing (maintaining the local phenotype), directional (selecting for a different mean phenotype), or diversifying (favoring two or more different phenotypes). ``
...
Specifically, by letting \selac's Grantham weighting term $G$, which we now assume is $\ge 0$,  to take on negative values, \selac now behaves as if there is a pessimal, rather than optimal, amino acid for the given site.
