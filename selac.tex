...
\marginpar{Does any one recall why we include mutation bias?  These models usually include nt specific mutation rates. JMB: Not sure either. Brian?}
...
\marginpar{Could you confirm this 73k value correct?  I get it from $61 \times 60 \times 20 = 73,200$. JMB: made the change. Should it not be $61 \times 61 \times 20 = 74,420$?} 
...
\marginpar{Didn't we compare these models to UNREST and find UNREST better?  If so we should mention this fact. JMB: I believe at one point we did, but i thought we went with UNREST because it fit with our model assumptions.}
...
That is, the rate of allele fixation is greater than \Nemu \marginpar{Shouldn't this be `$1/(2 \Ne)$ generations'? JMB: I agree -- did not make change until Brian confirms.}  and $\Nemu \ll 1$, such that evolution is mutation limited, codon substitutions only occur one nucleotide at a time and, as a result, the rate of change between any pair of codons that differ by more than one nucleotide is zero.
...
As traditionally done, we rescale our time units such that at equilibrium, one unit of branch length represents one expected neutral substitution per site.
More explicitly, $ S = \left(\sum_{i \in \text{codons}} \mu_i \pi_i\right)$.
\marginpar{I assume the $-$? in the previous version was a typo, correct?. JMB: No, it should be negative. Brian -- you agree?}
...
\marginpar{Are my corrections here correct? JMB: No. Brian -- you agree.?}
...
\marginpar{I thought we used 8 points. JMB: No. Takes way too long. Had to go with 4.}
...
\marginpar{Is this $<10$ accurate? JMB: Yes. Our yeast example is less than 10 species and it took forever.}
...
\marginpar{Can you fill in the XXX? JMB: I do not understand what is being communicated here.}
