...
%\footnote{mikeg: does this point warrant a figure?}
...
%As a result, our site independent assumption is violated and the fitting of our model becomes  more complex. 
...
%  W_i/W_j &\approx \exp\left[- q \left(C_1 + C_2 n\right) \frac{1}{n}\left(\sum_{p \in \setP} \sum_{k=1}^\kmax A_k \left(d\left(\aip,\aoptp\right)^k - d\left(\ajp,\aoptp\right)^k\right)\right) \psi \right]
%      &=  \exp\left[- q \left(C_1/n + C_2\right)\left(\sum_{p \in \setP} \sum_{k=1}^\kmax A_k \left(d\right(\aip,\aoptp\right)^k - d\left(\ajp,\aoptp\right)^k\right) \psi\right]
\end{align}
...
%   &\approx \frac{1-\exp\left[- q \left(\eta(\cveci)- \eta(\cvecj)\right) \psi b\right]}{1-\exp\left[- q \left(\eta(\cveci)- \eta(\cvecj)\right) \psi 2\Ne\right]}\\
...
%Taylor series formulation
%   &= \frac{1- \exp\left[- b\,q \left(C_1/n + C_2\right)\left(\sum_{k=1}^\kmax A_k \left(d\right(a_i,\aopt\right)^k - d\left(a_j,\aopt\right)^k\right)\left) \psi\right]}{1-\exp\left[- q \left(C_1/n + C_2\right)\left(\sum_{k=1}^\kmax A_k \left(d\right(a_i,\aopt\right)^k - d\left(a_j,\aopt\right)^k\right)\left) \psi \, 2 \, \Ne\right]},
\end{align*}
...
%\footnote{mikeg: Jeremy and Brian -- given that the formulation above suggests we can separate out $\psi$ and $\Ne$ when, with the current% dataset we cannot.  
...
%mikeg: okay, we will mention this fact in passing.
%}
...
%mikeg: Thanks for the clarification.
%I didn't realize these rates of evolution were so high.  
%This is interesting and worth exploring in future work.
%}
...
%\footnote{mikeg: Jeremy, avoid using $*$ for multiplication. Either leave it implicit $(a b)$ or use \\times, $(a \times b)$}
