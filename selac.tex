...
For one, they use only a single substitution matrix and are only applicable for situations in which the substitution process and shifts in the selective environment are intrinsicly, such as with positive or negative frequency dependent selection.
...
Lartillot's work \citep{LartillotAndPhilippe2004,RodrigueAndLartillot2014} greatly reduces the parameterization of the \citet{HalpernAndBruno1998} approach by grouping together sites with similar behaviors.
...
There are aspects of Lartillot's work that could be incorporated into our approach and vice versa.
However, our work differs in that we are attempting to provide an explicit model underlying this selection.
Conceptually, our work lies inbetween that of Lartillot's and Thorne's, where the latter is utlizing even more detailed models about protein structure as a means of linking amino acid substitutions and stabilizing selection.
One advantage of our approach over Thorne is the fact that there are a number of obvious generalizations.
