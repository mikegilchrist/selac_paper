...
In comparative methods (e.g., Butler and King 2004, O'Meara et al. 2006, Beaulieu et al. 2013) the number data points, $n$, is taken as the number of taxa.
...
An notable exception is the program SURFACE implemented by Ingram and Mahler (2013), which uses multiple characters and taxa.
...
Here, we have adopted and extended the simulation approach of Jhwueng et al. (2014) in order to examine a large set of different penalty functions and how well they approximate the remaining portion of Kullback-Liebler (KL) divergence between two models after accounting for the deviance (i.e., $-2\Lik$).
...
This is just one complexity; we expect there are many more, and so a deeper investigation of this issue is warranted.
...
\section{Table}
  ...
  \begin{table}[H]
    ...
    \end{tabular}
    \caption{Comparison of model fits using $\DeltaAICs$ and \AICw.}
