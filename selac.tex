...
\section{Discussion}
First and foremost as evolutionary biologists, we our goal is to use comparative sequence data in order to quantify the nature, strength, and, ultimately, shifts in the forces of natural selection relative to genetic drift and mutation.
...
Instead, allowing $G < 0$ implies that there is a pessimal, rather than optimal, amino acid for the site.
In this case the amino acid most \PC dissimilar than the pessimal amino acid represents the highest peak.
It is worth noting that allowing $G < 0$ is not equivalent to choosing a different amino acid as being optimal. 
For example, two amino acids could at opposite ends of \PC space could be equally dissimilar from the pessimal amino acid but would have similar fitness values and could represent alternative fitness peaks.
