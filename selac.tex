...
\title{Population genetics models with selection for phylogenetic inference}
\date{}
\begin{document}
\maketitle
...
%BCO: I am keeping paragraph labels below for now; should remove before submission
...
This results in the creation of a rich set of 21 such matrices (20 for the amino acids, and one for stop codons), with up to LARGE NUMBER unique rates, based on few parameters (one to six mutation rates, three weights on physiochemical distances, the cost of protein production, target functionality, and optimal amino acid at each site), which can be inferred from the data.
