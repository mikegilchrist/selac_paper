...
Specifically, we assume the strength of stabilizing selection for the optimal sequence, \aoptvec, is proportional to $\psi$, which we can estimate for each gene. 
...
Using both \DeltaAIC and \AICw as our measure of model support, the SelAC models perform extraordinarily better than GTR+$\Gamma$, GY94, or FMutSel0.
...
With respect to estimates of $\psi$ within SelAC, they were strongly correlated with two separate empirical measurements of gene expression (See Figure).
In other words, using only codon sequences our model can predict which genes have high or low expression levels. %\footnote{mikeg: Sooo why talk about codon sequences when we are only using the aa sequence?
%  On a  similar note, shouldn't we call this model SelA since we don't have selection on synonymous codon usage in the model yet? JMB: I just used SelAC as I was doing this, which can be changed. As for the other comment, not sure I follow. We are modelling codon evolution, so a user puts in codon data not amino acids and gets out gene expression estimates. That is the point I was trying to make.
%  BCO: Besides being the input data, we also use codons for the mutation model. It's not clear to me that we'd have similar performance if we were just using AA (remember how with JJ's original work we were making assumptions about the mutation process between AA that may not have been justified?).
%mikeg: Yeah, I forgot we were working fully at the codon level.}
...
In simulation, however, of all the parameters in the model, only $\alphag$ showed a consistent bias, in that the estimates were generally underestimated (see Supplemental).
...
Our estimates of these weights only strongly deviate from Grantham in regards to composition weight, $\alphac$, which is the distance between any two amino acids with regards to the ratio of noncarbon elements in the end groups to the number of side chains.
...
When we simulated the sequence for \emph{S. cervisieae}, starting from the ancestral sequence under both GTR+G and FMutSel0, the functionality of the simulated sequence moves away from the observed sequence, whereas SelAC remains near the functionality of the observed sequence (Figure XX).
In a way, this is somewhat unsurprising, given that both GTR+G and FMutSel0 are agnostic to the functionality of the gene, but it does highlight the improvement in biological realism in amino acid sequence evolution that our SelAC provides.
...
For instance, \emph{S. castellii} is simulated with consistently higher functionality than observed (Figure XX).
...
\begin{figure}[H]
  ...
  \caption{Maximum Likelihood Trees for (a) selac, (b) selac with uniform sensitivity $G = 1$, (c) GTR, (d) GY94, and (e) FMutSel0, (f) \citet{LartillotAndPhilippe2004}.}
  \label{fig:mleTrees}
\end{figure}
\item Model Adequacy Illustrations \label{fig:modelAdequacy} (Brian or Jeremy?)
\item Gene Expression Comparisons \label{fig:phiComparisons} (Jeremy now has this data)
  ...
  \item SelAC vs. Empirical  Measurements
    \begin{figure}[H]
      ...
      \caption{Comparison of log protein synthesis rate $\psi$ for \emph{S.~cerevisiae} as predicted by selac to empirical estimates from (a) ribosome profile footprint data \citep{IngoliaEtAl2009} and (b) mRNA abundance data \citep{HolstegeEtAl1998}.\footnote{are the \selac $\phi$ values rescaled in any way besides setting the substitution rate to 1?
}
      }\label{fig:selaVsPhi}
    \end{figure}
  \item GTR $\omega$ vs. $\phi$ Measurements
    \begin{figure}
      
      \caption{$\omega$ vs $\phi$ Figure goes here}\label{fig:omegaVsPhi}
    \end{figure}
  \end{enumerate}
\item Quantifying optimal AA \label{fig:optAA} (Mike once I get info from Jeremy)\footnote{mikeg: looked around, but can't find this data. 
    Ideally it would be in the format of: Gene, Psi, Position, LLik AA A, LLik AA B, LLik AA C, ...
}
  \begin{enumerate}
  \item Represent using varying point size in 3D physicochemical space
...
\subsection*{Tables}
  \begin{table}
    ...
    \end{tabular}
    \caption{Comparison of model fits using $\DeltaAIC$.}\label{table:modelFits}
  \end{table}
\section*{Discussion}
...
Perhaps more convicingly, we find that the target expression level $\psi$ and realized protein synthesis rate $\phi$ are reasonably well correlated  with laboratory measurements of gene expression ($\rho = 0.33-0.44$). and Figure \ref{fig:phiComparisons})).
...
As a result, we set $\alphav = 3.990 \times 10^{-4}$, %\footnote{mikeg: does Grantham really report 7 digits of precision? JMB: He gives 0.000399, I think the 333 comes from me and my direct calculation following Grantham's paper.
mikeg: I've removed the extra digits.}
the value originally estimated by Grantham, and recognize that our our estimates of $\alphac$ and $\alphap$ and $\psi$ are scaled relative to this choice for $\alphav$.
More specifically,
\begin{equation*}
...
This entire procedure is repeated six times or until the log-likelihood between successive rounds is less than a specified tolerance (the default is XXXX).\footnote{mikeg: This description is bound to cause a reviewer to ask what the differences were b/w the penultimate and ultimate rounds.
Can you look at what the differences were and change the criteria from 0 to something consistent with how you fitted the model to the Rokas data?}
...
Our "known" parameters under a given generating model were based on fitting SelAC to the 106 gene data set and phylogeny of \citet{RokasEtAl2003}.\footnote{mikeg: Should this be updated to reference the \citet{SalichosAndRokas2013} dataset?}
...
\begin{figure}[H]
  ...
  \caption{Comparison of ROC and SelAC estimates of $\phi$.  
    ROC estimates are inferred from genome scale patterns of synonymous codon usage driven by a combination of mutation bias and natural selection for synonymous codons with shorter pausing times which, as here, is scaled by gene expression $\phi$.
    See \citet{GilchristEtAl2015} for more details.
  }
\end{figure}
