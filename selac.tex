...
\marginpar{Does any one recall why we include mutation bias?  These models usually include nt specific mutation rates.}
...
Because \selac requires twenty families of $61 \times 61$ matrices, the number of parameters needed to implement \selac would, without further assumptions, be extremely large (i.e. on the order of 73,000 parameters).
\marginpar{Could you confirm this 73k value correct?  I get it from $61 \times 60 \times 20 = 73,200$.} 
...
\marginpar{Didn't we compare these models to UNREST and find UNREST better?  
If so we should mention this fact.}
...
More explicitly, $ S = 1/\left(\sum_{i \in \text{codons}} \mu_i \pi_i\right)$.
\marginpar{I assume the $-$? in the previous version was a typo, correct?}
Thus, the final mutation rate matrix is the original mutation rate matrix multiplied by $S$, and, on average, the expected rate of neutral evolution is 1 substitution per unit time.
\marginpar{Are my corrections here correct?}
...
This results in the creation of 20 \Qmatrix matrices, one for each amino acid and each with  $26,880$\marginpar{where does this 26,880 come from?} unique rates, based on few parameters (one to 11 mutation rates, two free Grantham weights, the cost of protein assembly, $A_1$ and $A_2$, the gene specific target functionality synthesis rate $\psi$, and optimal amino acid at each position $p$, \aoptp), which can either be specified \emph{a priori} or  estimated from the data.
\selac can be generalized to allow transitions between optimal amino acids as well as between codons, which would result in a $(20 \times 64) \times (20 \times 64) =  1344 \times 1344$ matrix.\marginpar{Is this worth mentioning?  If so why here instead of in Discussion?}
...
\marginpar{I thought we used 8 points.}
...
This entire four stage cycle is repeated six times, by which point the change in the log-\Lik values for our analysis was less than XXX between cycles.
\marginpar{Please replace XXX with appropriate numbers.}
...
\marginpar{Can you fill in the XXX?}
