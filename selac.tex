...
A number of researchers have used \PC based models to make inferences about selection for such properties \citet[e.g., see][]{Hughes1990,XiAndLi1998,WoolleyEtAl2003, McClellanAndMcCracken2001, BlazejEtAl2017}; however, in terms of tree and/or branch length reconstruction, \PC based codon models as originally introduced have rarely been used for empirical data.
\marginpar{Citing reviewer 2 suggestions: McClellan, Mackiewicz, Yang, Zhang, Goldstein, Higgs and presumably TamuriEtAl2009.
  These papers generally don't create the tree, but estimate substitutions given a tree.}
...
It is worth distinguishing this from the evolution of proteins to adapt to a shift at one site by compensatory changes at others \citep{ShahEtAl2015,PollockEtAl2012}, where a shift from a new amino acid back to the previous one becomes less likely with time.
...
\selac models also appeared to outperform, based on likelihood, and reported AIC and AICc from each program, the codon models in IQtree \cite{IQTree}. %citation
See Table \label{table:modelFitsAll} for results. %add table
...
\marginpar{Discuss \PC models here.}
...
Further, our model does not consider additional costs such as those imposed by misfoled proteins \citet{DrummondAndWilke2008}.
...
We assume single nucleotide changes only, despite evidence from \citet{KosiolEtAl2007, WhelanAndGoldman2004} that better fit can be accomplished by allowing instantaneous mutations of multiple nucleotides simultaneously.
...
Because \selac is built on very particular assumptions - the exact combination of \PC properties favored by selection matches that of an amino acid, sites evolve independently of one another, the cell's response to suboptimality is to produce more proteins, etc -- it is likely these assumptions are violated in many, if not most, cases.
...
In addition, other biological mechanisms could result in similar, if not identical, mathematical representations (\_\_Goldberg and Rabosky, Hisse\_\_) which, in turn, illustrates one limit to comparative sequence analysis.



There are also deficiencies in our implementation.
Though reasonable to use for a given topology with 10's of species, it is currently too slow for practical use for tree search.
...
Most work in this area use a given tree to make inferences about the nature or distribution of selection coefficients \citet[e.g.]{Hughes1990,XiAndLi1998,WoolleyEtAl2003, McClellanAndMcCracken2001,BlazejEtAl2017}.
\marginpar{mikeg: note citations here mirror those made in intro.}
While the work of \citet{KoshiEtAl1999,DimmicEtAl2000,KoshiAndGoldstein2001,RobinsonEtAl2003,LartillotAndPhilippe2004,ThorneEtAl2012,RodrigueAndLartillot2014} does involve tree inferences, are all models of constant, stabilizing selection, \selac can be generalized further to include diversifying selection.
...
Due to its mechanistic nature, \selac can also be extended to include changes in the optimal/pessimal sequence over a phylogeny using a hidden Markov modelling approach \citet[][for a non Markov approach see \citet{TamuriEtAl2009}]{TuffleySteel1998,Pennyetal2001,Whelan2008}.
...
Such approaches would be challenging using non-mechanistic and parameter rich models which infer up to 19 different parameters, one for each amino acid, per site such as those introduced by \citet{HalpernAndBruno1998,TamuriEtAl2012,TamuriEtAl2014} \citep[but see][for an elegant means of reducing the number of different site models]{RodrigueAndLartillot2014}.
...
We also compared \selac models with 195 codon models in IQtree. %CITE THIS SOON
This is popular software implementing recent (i.e., \_\_\_\_\_\_) models of evolution, including codon models. %CITATIONS
...
Further, while there are no identical models in \selac and IQtree, similar models have similar likelihoods, suggesting the log-likelihood values between \selac and IQtree are comparable (as one would expect).
...
\section{Table}
  ...
  \begin{table}[H]
    ...
    \caption{Comparison of maximum likelihood fits for \selac and commonly used models based on negative log likelihood ($-ln(\Lik)$), AIC, AICc, and \AICw from analyses of 100 selected genes from 6 yeast taxa \citet{SalichosAndRokas2013}.
      ...
      For the comparison between the different \selac and 192 other models fitted using IQTree \citep{Nguyen2015} see Table \ref{table:modelFitsAll} in the Supporting Materials.
...
\include{Tables/selac-and-iqtree-table}
