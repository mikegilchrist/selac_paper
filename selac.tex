...
In fact, $\omega$ showed similar, though slightly reduced correlations, with the same empirical estimates of gene expression described above (See Figure).
...
In a way, this is somewhat unsurprising, given that both GTR+G and FMutSel0 are agnostic to the functionality of the gene, but it does highlight the improvement in biological realism in codon evolution that our SelAC provides.
...
\footnote{mikeg: I think you're confusing two points here.  Because selection is scaled by $\psi$, a small decrease in functionality could lead to a large decrease in fitness if the gene is expressed at a high enough level.
The figures we've talked about looking at the distribution of fitness effects should be able to illustrate this point.
}
\end{enumerate}
