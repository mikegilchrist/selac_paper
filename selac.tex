...
\section{Discussion}
One goal of evolutionary biology is to quantify the nature, strength, and, ultimately, shifts in the forces of natural selection relative to genetic drift and mutation using genetic data.
\omega based models have been important in helping to achieve this. 
However, their simplicity, which has benefits in both interpretability and speed, also has some costs.
For example, their assumption that there is selection is about resistant to amino acid change, rather than for amino acid identity, limits the biological cases for which it is appropriate.
...
The work of Lartillot and colleagues \citep{LartillotAndPhilippe2004,RodrigueAndLartillot2014} greatly reduces the parameterization of the \citet{HalpernAndBruno1998} approach by grouping together sites with similar behaviors. 
While an important advance, these models are essentially descriptive in nature; they have no underlying model for the nature of the stabilizing selection profiles they are estimating. 
There are aspects of Lartillot et al.'s work that could be incorporated into our approach, and vice versa. 
However, our framework differs in that we are attempting to provide an explicit model underlying this selection. 

Conceptually, our work lies between that of Lartillot et al. and Thorne et al., where the latter is utilizing even more detailed models about protein structure as a means of linking amino acid substitutions and stabilizing selection. 
One advantage of our proposed framework is the fact that there are a number of obvious generalizations. 
For example, if one views stabilizing and diversifying selection as opposite ends of the spectrum of how selection in nature behaves, then allowing the $G$ parameter to take on negative values provides a means of modeling this spectrum. 
More specifically, letting $G < 0$ implies that there is a pessimal, rather than optimal, amino acid for the site. In this case, the amino acid in \PC qualities most dissimilar to the pessimal amino acid represents the highest fitness peak. 
% Yeah, that's true. But is there a single biological realistic case? Or even a whiff of a case? Even for stuff like binding sites for viruses or other nasties, it's not that being as different as possible is a criterion -- still want it water soluble, for example. I'd cut the whole G < 0 section - discussion is already long. -BCO
It is worth noting that allowing $G < 0$ is not equivalent to choosing a different amino acid as being optimal. 
To see this, imagine a focal amino acid with two alternative amino acids equally distant from it in \PC but in opposite directions. 
...
This ability to extend our model and, in turn, sharpen our thinking about the nature of natural selection on amino acid sequences, illustrates the value of moving from descriptive to more mechanistic models in general, particularly in area of molecular phylogenetics.

As phylogenetic methods become ever more ubiquitous in biology, and data set size and complexity increase, there is a need for more complex and realistic models \citep{GoldmanEtAl1996,ThorneEtAl1996,GoldmanEtAl1998,HalpernAndBruno1998,LartillotAndPhilippe2004}. 
...
Our work here follows in this tradition, but includes some key advances. 
First, by nesting a model of a sequence's cost-benefit function \Cost/\Func within allows us to formulate and test more specific biological hypotheses. 
...
Our model is built on a formal description of a sequence's \Cost/\Func, which make modifying our assumptions relatively straightforward. 
...
For example, it is possible to extend our model to include a hidden Markov modelling (HMM) approach, which would allow for shifts in the optimal/pessimal amino sequence along any given branch in the phylogeny. 
