...
Instead, $\omega$ based models only reasonably approximate a subset of scenarios such as over/underdominance or positive/negative frequency dependent selection \citep{HughesAndNei1988,Nowak2006} where selection is perfectly symmetrical.
...
Importantly, these costs can be computed to varying degrees of realism.
For example, we have previously presented models of protein synthesis costs that, alternatively, take into account the cost of ribosome pausing \citep{ShahAndGilchrist2011} or premature termination errors \citet{GilchristAndWagner2006,Gilchrist2007,GilchristEtAl2009}.
...
Nevertheless, we believe our cost-benefit approach to be substantial advance of the more simplistic $\omega$ models and lays the foundation for more realistic work in the future.

For example, by assuming there is an optimal amino acid for each site naturally leads to a non-symmetrical and, thus, more cogent model of protein sequence evolution.
...
Further, our model does not assume the system is always at the optimum or pessimum point of the fitness landscape, as occurs when $\omega < 1$ or $>1$, respectively.



Importantly, the cost-benefit approach we develop here allows us to link the strength of selection on a protein sequence to its gene's expression level.
...
Unlike $\omega$ based approaches, because we make inferences about things other than branch length and tree, we can validate the assumptions indirectly by comparing our inferences to other empirical data, such as we do here with data on protein synthesis rates.
More generally, our assumptions lead to mechanstic and, thus, testable hypothesis about the relationship between mutation, protein function, gene expression, and rates of evolution.
