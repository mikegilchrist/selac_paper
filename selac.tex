...
With respect to estimates of $\phi$ within SelAC, they were strongly correlated with two separate empirical measurements of gene expression (See Figure \ref{fig:PhivsROC}; Figure S1-S2\footnote{mikeg: Please add these figures. JMB: I did. See Supplemental materials at bottom of this document.}).
...
\begin{figure}[H]
  ...
  \caption{Comparisons between estimates of $\phi$ obtained from \selacplusgamma and the predicted gene expression from the ROC SEMPER model (\citet{GilchristEtAl2015} for individual taxa across the 100 selected genes from the yeast data set of Salichos and Rokas (2013).
  The equations in the upper left hand corner of each panel represent the regression fit and correlation coefficient.
  } 
  \label{fig:PhivsROC}
\end{figure}
\footnote{mikeg: ROC is NOT an 'empirical' based estimate of gene expression.  
  The mRNA and microarray data should be used instead and this figure should be put in the supporting materials. JMB: Realized I made this mistake yesterday. I thought I had edited that stuff out. Not seeing some of my changes in this version. Will fix again now.} 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{FIGURE_2_MutSelOmega_vs_Us_ROC_Scer_only.pdf}
  \caption{Comparisons between $\omega$, which is the nonsynonymous/synonymous mutation ratio in FMutSel0, $\phi$ obtained from \selacplusgamma (a), and two empirical estimates of gene expression for \emph{S. cervisaea} across the 100 selected genes from the yeast data set of Salichos and Rokas (2013).  
...
\end{figure}
\footnote{mikeg: For (a) shoudln't it be $\psi$, which is not genome specific, rather than $\phi$, which is.  
      Is the RNA seq and ROC data for brewer's yeast? JMB: Good catch on both accounts. Fixed the caption, will fix the figure.
  } 
...
Our estimates of $\phi$ are positively correlated with both empirical, the two empirical measurements also strongly correlate with each other (Figure 1 \ref{fig:PhivsROC}; Figure S1-S2 \ref{fig:PhivsEmpirical}, \ref{fig:ROCvsEmpirical}) 
This suggest a high degree of consistency among all three measures regarding the expression level of a gene.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{FIGURE_S1_SelACwG_vs_Empirical_by_spp.pdf}
  \caption{Comparisons between estimates of $\phi$ obtained from SelAC+$\Gamma$ and direct measurements of expression for individual taxa across the 100 selected genes from the yeast data set of Salichos and Rokas (2013).
  	Gene expression was measured using either RNA-Seq (a-c) or Microarray chips (d).
    The equations in the upper left hand corner of each panel represent the regression fit and correlation coefficient.
  } 
...
\begin{figure}[H]
  ...
  \caption{Comparisons of predicted gene expression from the ROC SEMPER model (\citet{GilchristEtAl2015} and direct measurements of expression from RNA-Seq or Microarray data for individual taxa across the 100 selected genes from the yeast data set of Salichos and Rokas (2013).
      The equations in the upper left hand corner of each panel represent the regression fit and correlation coefficient.
  } 
...
However, the inclusion of $\alphag$ into the model, appears to generally increase values of $\phi$ and generally improves the ability to recover the optimal amino acids for even the gene with the lowest baseline $\phi$.
...
\footnote{JMB: I swear there is a paper that shows gamma in nucleotide models are generally underestimated -- am I nuts? Probably.
}
