...
\access{Version accepted Nov.~14 2018 with requested minor revisions.}
...
\ifthenelse{1=0}{%include supporting materials}
\clearpage
...
Additional materials for: Beaulieu \emph{et al.}~(2019). Population Genetics Based Phylogenetics Under Stabilizing Selection for an Optimal Amino Acid Sequence: A Nested Modeling Approach \emph{Molecular Biology and Evolution}.
...
\subsection{Table}
\newcommand{\tableLabelRow}{Rank&Model&$-\ln(\Lik)$&df&AIC&AICc&\DeltaAIC&\DelA}
%\begin{table}
\begin{longtable}{rlrrrrrr}
\caption{Comparison of \selac to 163 other phylogenetic models to 100 selectedd.
See Table \ref{table:modelFits} for more details
Models of Rank 1-4, 88, and 166 were fitted using the \selac software package.A.
Note that there are no models in common between IQTree and \selac.
\selac's GY and IQTree's GY+F are the most similar.
However, \selac's GY uses a mutation model to generate expected codon frequencs.
% NB both selac and IQ tree drop cells with missing data.
