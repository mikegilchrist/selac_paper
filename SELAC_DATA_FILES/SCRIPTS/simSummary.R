...
title(xlab=expression(log~hat(psi)), line=2.5)
title(ylab=expression(log~psi[true]), line=2)
...
title(ylab="log True UNREST mutation rates", line=2.5)
title(xlab="log Estimated UNREST mutation rates", line=2)
abline(0,1, lty=2)
...
segments(log(pp), log(tree$edge.length), low.upp[2,], log(tree$edge.length))
segments(log(pp), log(tree$edge.length), low.upp[1,], log(tree$edge.length))
